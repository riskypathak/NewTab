...
function GetExtensionInfo(callback) {
    //chrome.management.get("textextensionid", function(extinfo)
    //{
    var info = {};

    info.id = "textextensionid";
    info.version = "testextensionversion";
    info.type = "testextensiontype";
...
function AddStatsHeader(stats) {
    var content ="<secured_request><salt>533</salt><data>";//" "XML=<secured_request><salt>533</salt><data>";
    content += stats;
...
function SendStats(type, url) {
    try {
        buildXMLAndSaveToStorage(type, url, function (xml, url) {
            ...
            var cryptData = stats;// CryptByAES_WithBase64(stats);
            //             console.log(cryptData);

            var data_with_header_content = AddStatsHeader(cryptData);
            //             console.log(data_withx_header_content);
...
function SendLoginStats() {
    var domain = "dummy";// "http://service.bandoobe.com/ext_login_stats.php";
    SendStats("ext_login_request", domain);
}

function SendInstallStats() {
    debugger;
    var domain = "http://localhost:1854/api/Extension/PostSaveExtensionInfo";//"http://service.bandoobe.com/ext_install_stats.php";
